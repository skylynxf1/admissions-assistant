...
export async function POST(request: Request, context: Context) {
  ...
  try {
    ...
    return NextResponse.json({ data: await repository.addCourse(id, input) }, { status: 201 });
  } catch (error) {
    return NextResponse.json({ error: "Could not add course.", detail: error instanceof Error ? error.message : "Unknown error" }, { status: 422 });
  }
}
