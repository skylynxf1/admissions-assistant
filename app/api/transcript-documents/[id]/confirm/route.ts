...
export async function POST(request: Request, context: Context) {
  ...
  try {
    ...
    return NextResponse.json({ data: await repository.confirm(id) });
  } catch (error) {
    return NextResponse.json({ error: "Could not confirm transcript.", detail: error instanceof Error ? error.message : "Unknown error" }, { status: 422 });
  }
}
