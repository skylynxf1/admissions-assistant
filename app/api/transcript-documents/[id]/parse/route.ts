...
export async function POST(request: Request, context: Context) {
  ...
  try {
    ...
    const processed = await processStoredTranscript(repository, id);
    return NextResponse.json({ data: { ...processed, detail: await repository.getDetailed(id) } });
  } catch (error) {
    return NextResponse.json({ error: "Transcript processing failed.", detail: error instanceof Error ? error.message : "Unknown error" }, { status: 422 });
  }
}
