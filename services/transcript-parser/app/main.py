...
async def parse_transcript(
    ...
    parser: Annotated[str, Query(pattern="^(docling|marker|pymupdf)$")] = "docling",
) -> ParsedDocument:
    if file.content_type != "application/pdf" and not (file.filename or "").lower().endswith(
        ".pdf"
    ):
        raise HTTPException(
            status_code=415, detail={"code": "corrupt_pdf", "detail": "A PDF file is required."}
        )
    payload = await file.read()
    if not payload:
        raise HTTPException(
            status_code=422, detail={"code": "empty_pdf", "detail": "The PDF is empty."}
        )
    if len(payload) > 15 * 1024 * 1024:
        raise HTTPException(
            status_code=413, detail={"code": "parser_failed", "detail": "The PDF exceeds 15 MB."}
        )
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=422,
                detail={"code": "corrupt_pdf", "detail": f"The PDF is corrupt: {error}"},
            ) from error
        ...
        if parser == "marker":
            raise HTTPException(
                ...
                detail={
                    ...
                    "detail": "Marker is an optional adapter and is not installed in this image.",
                },
            )
        ...
        try:
            ...
            raise HTTPException(
                status_code=422, detail={"code": "parser_failed", "detail": str(error)}
            ) from error
    finally:
