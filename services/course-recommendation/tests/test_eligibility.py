...
def test_and_prerequisite_group_requires_every_condition():
    groups = [group("all", GroupType.ALL, [
...
def test_or_prerequisite_group_accepts_either_course():
    groups = [group("any", GroupType.ANY, [
...
def test_minimum_count_group_enforces_configured_count():
    groups = [group("choose-two", GroupType.MIN_COUNT, [
    ...
    assert evaluate(groups, [CompletedCourse(course_id="a"), CompletedCourse(course_id="c")]).eligible is True


def test_minimum_grade_requirement_fails_unknown_or_low_grade():
    groups = [group("grade", GroupType.ALL, [
        condition(
            "grade", "grade-condition", ConditionType.COURSE,
            prerequisite_course_id="prog1", minimum_grade_points=2.0,
        )
    ])]
...
def test_concurrent_enrollment_is_reported_separately():
    groups = [group("concurrent", GroupType.ALL, [
        condition(
            "concurrent", "concurrent-condition", ConditionType.COURSE,
            prerequisite_course_id="calc2", may_be_concurrent=True,
        )
    ])]
...
def test_placement_test_alternative_is_supported_and_explained():
    groups = [group("placement", GroupType.ANY, [
        condition("placement", "course", ConditionType.COURSE, prerequisite_course_id="calc1"),
        condition(
            "placement", "test", ConditionType.PLACEMENT,
            placement_test_code="ALEKS", minimum_placement_score=75,
        ),
    ])]
...
def test_program_admission_and_permission_are_not_assumed():
    groups = [group("admission", GroupType.ALL, [
        condition(
            "admission", "program", ConditionType.PROGRAM_ADMISSION,
            admitted_program_id="engineering",
        ),
        condition(
            "admission", "permission", ConditionType.INSTRUCTOR_PERMISSION,
            permission_required=True, raw_requirement_text="Instructor permission",
        ),
    ])]
...
def test_complex_course_rule_matches_specification():
    groups = [
        group("math", GroupType.ANY, [
            condition("math", "calc", ConditionType.COURSE, prerequisite_course_id="calc1"),
            condition(
                "math", "placement", ConditionType.PLACEMENT,
                placement_test_code="ALEKS", minimum_placement_score=75,
            ),
        ]),
        group("programming", GroupType.ALL, [
            condition(
                "programming", "prog", ConditionType.COURSE,
                prerequisite_course_id="prog1", minimum_grade_points=2.0,
            )
        ]),
    ]
