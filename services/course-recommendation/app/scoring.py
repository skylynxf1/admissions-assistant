...
class WeightedRecommendationScorer:
    ...
    def score(
        ...
        *,
        ...
        helped_priority_value = sum(
            1 / priority for priority in set(features.scenario_priorities_helped)
        )
        ...
        raw = {
            ...
            "graduation_acceleration_score": config.graduation_acceleration_weight
            * acceleration_signal,
            "infrequent_offering_score": config.infrequent_offering_weight
            * float(features.offered_infrequently),
            "uncertain_equivalency_penalty": -config.uncertain_equivalency_penalty
            * uncertain_signal,
            "dead_end_penalty": -config.dead_end_penalty * features.dead_end_risk,
            "duplicate_credit_penalty": -config.duplicate_credit_penalty
            * float(features.duplicate_credit_risk),
        }
        positive_capacity = (
            sum(
                [
...
def usefulness_label(features: CourseRecommendationFeatures) -> str:
    if (
        features.equivalency_confidence in {"low", "unknown"}
        and features.selected_institutions_helped == 0
    ):
...
def risk_level(
    dead_end_risk: float, duplicate_credit_risk: bool, equivalency_confidence: str
) -> str:
    if (
        duplicate_credit_risk
        or dead_end_risk >= 0.67
        or equivalency_confidence in {"low", "unknown"}
    ):
        return "HIGH"
...
def deterministic_reasons(features: CourseRecommendationFeatures) -> list[str]:
    ...
    if features.selected_institutions_helped:
        institution_count = features.selected_institutions_helped
        reasons.append(
            f"Has accepted applicability records at {institution_count} selected institution"
            f"{'s' if institution_count != 1 else ''}."
        )
    ...
    if features.estimated_graduation_terms_saved:
        saved_terms = features.estimated_graduation_terms_saved
        reasons.append(
            f"Taking it now may conservatively avoid up to {saved_terms} term"
            f"{'s' if saved_terms != 1 else ''} of prerequisite delay."
        )
    if features.offered_infrequently:
        reasons.append(
            "The course is not offered every standard term, increasing the value of taking it now."
        )
...
def deterministic_warnings(features: CourseRecommendationFeatures, concurrent: bool) -> list[str]:
    ...
    if features.equivalency_confidence != "high":
        warnings.append(
            f"Transfer applicability confidence is {features.equivalency_confidence}; "
            "verify the cited source records."
        )
    if features.duplicate_credit_risk:
        warnings.append(
            "This course may duplicate credit already completed; it is retained only "
            "for transparent scoring."
        )
    if features.dead_end_risk >= 0.65:
