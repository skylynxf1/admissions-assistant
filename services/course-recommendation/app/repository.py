...
class RecommendationRepository(Protocol):
    ...
    async def save_cached(
        self, response: RecommendationResponse, weight_version: str, academic_version: str
    ) -> None: ...
...
class InMemoryRecommendationRepository:
    ...
    async def save_cached(
        ...
        self.cache[(response.scenario_id, response.target_term, response.scenario_fingerprint)] = (
            response
        )
...
class SupabaseRecommendationRepository:
    ...
    async def load_dataset(self, scenario_id: str) -> ScenarioDataset:
        ...
        targets = (
            self._rows("planning", "scenario_targets").eq("scenario_id", scenario_id).execute().data
        )
        selected_programs = [
        ...
        student_rows = (
            self._rows("student", "student_courses")
            .eq("user_id", scenario_row["user_id"])
            .execute()
            .data
        )
        student_courses = [
            self._student_course(row, courses, current_institution_id) for row in student_rows
        ]
        weight_rows = (
            self._rows("planning", "recommendation_weight_configs")
            .eq("active", True)
            .limit(1)
            .execute()
            .data
        ...
        weight_config = (
            RecommendationWeightConfig(**self._only(RecommendationWeightConfig, weight_rows[0]))
            if weight_rows
            else RecommendationWeightConfig()
        )
        ...
        scenario = PlanningScenario(
            ...
            max_credits=float(scenario_row["max_credits"])
            if scenario_row.get("max_credits") is not None
            else None,
        ...
        data_version = self._data_version(
            course_rows, group_rows, condition_rows, requirement_rows, equivalency_rows
        )
    ...
    async def get_cached(
        ...
        return (
            RecommendationResponse.model_validate(rows[0]["response"]).model_copy(
    ...
    @staticmethod
    def _only(
        model: type[Any], row: dict[str, Any], exclude: set[str] | None = None
    ) -> dict[str, Any]:
    ...
    def _offering_status(row: dict[str, Any]) -> OfferingStatus:
        ...
        return (
            OfferingStatus(normalized)
            if normalized in OfferingStatus._value2member_map_
            else OfferingStatus.UNKNOWN
        )
    ...
    def _student_course(
        ...
        status = (
            StudentCourseStatus(raw_status)
            if raw_status in StudentCourseStatus._value2member_map_
            else StudentCourseStatus.COMPLETED
        )
        ...
        return StudentCourse(
            ...
            if row.get("credits_earned") is not None
            else None,
            grade=row.get("grade"),
    ...
    def _program_requirement(row: dict[str, Any]) -> ProgramRequirement:
        ...
        return ProgramRequirement(
            ...
            minimum_credits=float(row["minimum_credits"])
            if row.get("minimum_credits") is not None
            else None,
