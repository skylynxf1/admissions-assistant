...
const summerTerm: SimulatorTerm = { id: "summer-2027", label: "Summer 2027", note: "Optional" };
...
function buildTerms(plannedCourses: PlannedCourse[], attendSummer: boolean) {
  ...
  plannedCourses.forEach((course) => {
    if (!terms.some((term) => term.id === course.termId)) terms.push({ id: course.termId, label: course.termLabel, note: "Saved plan" });
  });
  return terms;
}

function buildSchedule(terms: SimulatorTerm[], plannedCourses: PlannedCourse[]) {
  const schedule = Object.fromEntries(terms.map((term) => [term.id, [] as string[]]));
  plannedCourses.forEach((course) => { schedule[course.termId] = [...(schedule[course.termId] ?? []), course.id]; });
  return schedule;
}
...
export function DashboardView() {
  const {
    targets, setTargets, prioritySchoolId, setPrioritySchoolId, scenario, setScenario,
    updateScenario, transcript, setTranscript, analysis, analysisStatus,
  } = useApp();
  ...
  useEffect(() => {
    const nextTerms = buildTerms(scenario.plannedCourses, scenario.attendSummer);
    setTerms(nextTerms);
    setSchedule(buildSchedule(nextTerms, scenario.plannedCourses));
  }, [scenario.plannedCourses, scenario.attendSummer]);
  ...
  const courseById = useMemo(() => {
    ...
    (analysis?.recommendations ?? []).forEach((course) => map.set(course.id, course));
    scenario.plannedCourses.forEach((course) => map.set(course.id, course));
    return map;
  }, [analysis?.recommendations, scenario.plannedCourses]);

  if (!analysis && analysisStatus === "analyzing") return <PlannerLoading />;
  if (!analysis) return <main className="mx-auto max-w-xl px-5 py-20"><EmptyState title="Choose at least one major" description="Add a university and major so the simulator can calculate eligibility and build a course path." /><div className="mt-5 text-center"><Link href="/targets" className="primary-button">Choose destinations</Link></div></main>;
  ...
  const plannedCoursesFromSchedule = (nextSchedule: Record<string, string[]>) => Object.entries(nextSchedule).flatMap(([termId, courseIds]) => {
    ...
    return courseIds.flatMap((courseId) => {
      ...
      if (!course) return [];
      return [{
        id: course.id, course: course.course, title: course.title, credits: course.credits,
        termId, termLabel: term.label, satisfies: course.satisfies, source: course.source ?? "recommended",
      } satisfies PlannedCourse];
    });
  });

  const commitSchedule = (next: Record<string, string[]>) => {
    setSchedule(next);
    updateScenario({ plannedCourses: plannedCoursesFromSchedule(next) });
  };
  ...
  const dropCourse = (event: React.DragEvent, termId: string) => {
    ...
    const next = Object.fromEntries(Object.entries(schedule).map(([id, ids]) => [id, ids.filter((item) => item !== courseId)]));
    next[termId] = [...(next[termId] ?? []), courseId];
    commitSchedule(next);
  };

  const removeCourse = (termId: string, courseId: string) => commitSchedule({ ...schedule, [termId]: (schedule[termId] ?? []).filter((id) => id !== courseId) });

  const addToOpenTerm = (courseId: string) => {
    const nextTerm = [...terms].sort((a, b) => (schedule[a.id]?.length ?? 0) - (schedule[b.id]?.length ?? 0))[0];
    if (!nextTerm) return;
    const next = Object.fromEntries(Object.entries(schedule).map(([id, ids]) => [id, ids.filter((item) => item !== courseId)]));
    next[nextTerm.id] = [...(next[nextTerm.id] ?? []), courseId];
    commitSchedule(next);
  };

  const addCustomCourse = (course: PlannedCourse) => {
    const next = { ...schedule, [course.termId]: [...(schedule[course.termId] ?? []), course.id] };
    setSchedule(next);
    updateScenario({ plannedCourses: [...scenario.plannedCourses.filter((item) => item.id !== course.id), course] });
  };

  const addSummer = () => {
    if (!terms.some((term) => term.id === summerTerm.id)) setTerms((current) => [...current, summerTerm]);
    setSchedule((current) => ({ ...current, [summerTerm.id]: current[summerTerm.id] ?? [] }));
    updateScenario({ attendSummer: true });
  };
  ...
  const saveComparison = () => {
    ...
    setComparisons((current) => [...current, {
      id: `plan-${Date.now()}`,
      label: `Plan ${count}: ${prioritySchool?.shortName ?? "Scenario"}`,
      createdAt: new Date().toISOString(),
      prioritySchoolId,
      targets: structuredClone(targets),
      scenario: structuredClone(scenario),
      transcript: structuredClone(transcript),
      summary: structuredClone(analysis.simulationSummary),
    }]);
  };

  const restoreComparison = (plan: PlanComparison) => {
    setTargets(structuredClone(plan.targets));
    setPrioritySchoolId(plan.prioritySchoolId);
    setScenario(structuredClone(plan.scenario));
    setTranscript(structuredClone(plan.transcript));
    setTab("plan");
  };
  ...
  const renderWidget = (widgetId: WidgetId) => {
    ...
    if (widgetId === "library") {
      ...
      content = <div className="space-y-3">{analysis.recommendations.slice(0, 4).map((course) => <div key={course.id} className="rounded-xl bg-teal-50/70 p-3"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-bold text-teal-800">{course.course}</p><p className="mt-0.5 text-sm font-semibold text-slate-900">{course.title}</p></div><button onClick={() => addToOpenTerm(course.id)} className="grid size-7 shrink-0 place-items-center rounded-lg bg-white text-teal-700 shadow-sm" aria-label={`Add ${course.course} to plan`}><Plus className="size-3.5" /></button></div><p className="mt-2 text-[11px] leading-4 text-slate-600">Meets {course.satisfies.join(" + ")}.</p></div>)}</div>;
    } else if (widgetId === "prerequisites") {
      content = <div className="space-y-4">{analysis.prerequisiteChains.slice(0, 3).map((chain) => <div key={chain.id}><p className="text-xs font-semibold text-slate-800">To reach {chain.targetCourse}</p><div className="mt-2 flex flex-wrap items-center gap-1.5">{chain.steps.map((step, index) => <div key={step.course} className="flex items-center gap-1.5"><span className={`rounded-lg px-2 py-1.5 text-[10px] font-semibold ${step.state === "complete" ? "bg-emerald-50 text-emerald-700" : step.state === "in-progress" ? "bg-blue-50 text-blue-700" : "bg-slate-100 text-slate-600"}`}>{step.course}</span>{index < chain.steps.length - 1 && <span className="text-slate-300">→</span>}</div>)}</div></div>)}</div>;
    } else {
      const summary = analysis.simulationSummary;
      content = <div className="grid grid-cols-2 gap-2">{[
        ["Earned", analysis.creditSummary.earned], ["Transferable", summary.projectedTransferableCredits],
        ["Remaining", summary.estimatedRemainingCredits], ["GE progress", `${summary.generalEducationPercent}%`],
      ].map(([label, value]) => <div key={label as string} className="rounded-xl bg-slate-50 p-3 text-center"><p className="text-xl font-semibold tracking-tight text-slate-900">{value}</p><p className="mt-1 text-[10px] text-slate-400">{label}</p></div>)}</div>;
    }
  ...
  return <>
    <div className="print-hide sticky top-16 z-30 border-b border-slate-200 bg-white/95 backdrop-blur-xl">
      <div className="mx-auto flex max-w-[1440px] flex-col gap-3 px-5 py-3 xl:flex-row xl:items-center lg:px-8">
        <div className="flex min-w-0 items-center gap-3 xl:mr-3"><div className="grid size-9 shrink-0 place-items-center rounded-xl text-xs font-bold text-white" style={{ backgroundColor: prioritySchool?.color }}>{prioritySchool?.shortName.slice(0, 2).toUpperCase()}</div><div className="min-w-0"><p className="truncate text-sm font-semibold text-slate-900">{prioritySchool?.shortName}</p><p className="truncate text-[10px] text-slate-400">{selectedMajors.map((major) => major?.name).join(" · ")}</p></div></div>
        <nav className="flex flex-1 gap-1 overflow-x-auto">{tabs.map(({ id, label, Icon }) => <button key={id} onClick={() => setTab(id)} className={`flex shrink-0 items-center gap-1.5 rounded-lg px-3 py-2 text-xs font-semibold transition ${tab === id ? "bg-slate-900 text-white" : "text-slate-500 hover:bg-slate-100"}`}><Icon className="size-3.5" /> {label}{id === "compare" && comparisons.length > 0 && <span className="rounded-full bg-white/15 px-1.5 text-[9px]">{comparisons.length}</span>}</button>)}</nav>
        <div className="flex flex-wrap items-center gap-2">
          <button onClick={() => setSchoolPanelOpen(true)} className="secondary-button !min-h-9 !px-3 !py-1.5"><School className="size-3.5" /> Schools & majors</button>
          <button onClick={() => setRecordOpen(true)} className="secondary-button !min-h-9 !px-3 !py-1.5"><FilePenLine className="size-3.5" /> Record</button>
          <button onClick={() => setSettingsOpen(true)} className="secondary-button !min-h-9 !px-3 !py-1.5"><Settings2 className="size-3.5" /> Scenario</button>
          <button onClick={saveComparison} className="secondary-button !min-h-9 !px-3 !py-1.5"><GitCompareArrows className="size-3.5" /> Save plan</button>
          <button onClick={() => window.print()} className="primary-button !min-h-9 !px-3 !py-1.5"><Download className="size-3.5" /> Export PDF</button>
        </div>
      </div>
    </div>
    ...
    <main className="mx-auto max-w-[1440px] px-5 pb-28 pt-7 lg:px-8">
      {tab === "plan" && <>
        ...
        <section className="card mt-5 overflow-hidden">
          <div className="flex flex-col gap-3 border-b border-slate-100 px-5 py-4 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="font-semibold text-slate-900">Quarter plan</h2><p className="mt-1 text-xs text-slate-500">Drop a course into any term. Credit limits are checked automatically.</p></div><div className="flex gap-2"><button onClick={() => setCustomCourseOpen(true)} className="secondary-button !min-h-9 !px-3 !py-1.5"><Plus className="size-3.5" /> Add course</button><button onClick={addSummer} className="secondary-button !min-h-9 !px-3 !py-1.5"><Plus className="size-3.5" /> Add summer</button></div></div>
          <div className={`grid gap-3 p-4 ${terms.length > 3 ? "md:grid-cols-2 xl:grid-cols-4" : "md:grid-cols-3"}`}>
            {terms.map((term) => {
              const courseIds = schedule[term.id] ?? [];
              const credits = courseIds.reduce((sum, id) => sum + (courseById.get(id)?.credits ?? 0), 0);
              const overloaded = credits > scenario.preferredCreditLoad;
              return <div key={term.id} data-term-id={term.id} onDragOver={(event) => event.preventDefault()} onDrop={(event) => dropCourse(event, term.id)} className={`min-h-64 rounded-2xl border border-dashed p-3 transition ${overloaded ? "border-amber-400 bg-amber-50/50" : "border-slate-300 bg-slate-50/70 hover:border-teal-400 hover:bg-teal-50/40"}`}>
                <div className="flex items-start justify-between"><div><h3 className="text-sm font-semibold text-slate-900">{term.label}</h3><p className="mt-0.5 text-[10px] text-slate-400">{term.note}</p></div><span className={`rounded-full bg-white px-2 py-1 text-[10px] font-semibold shadow-sm ${overloaded ? "text-amber-700" : "text-slate-500"}`}>{credits}/{scenario.preferredCreditLoad} cr</span></div>
                <div className="mt-3 space-y-2">{courseIds.map((courseId) => {
                  const course = courseById.get(courseId);
                  if (!course) return null;
                  return <div key={courseId} data-planned-course-id={courseId} draggable onDragStart={(event) => startCourseDrag(event, courseId)} className="group rounded-xl border border-slate-200 bg-white p-3 shadow-sm"><div className="flex items-start gap-2"><GripVertical className="mt-0.5 size-3.5 text-slate-300" /><div className="min-w-0 flex-1"><p className="text-xs font-bold text-teal-700">{course.course}</p><p className="mt-0.5 truncate text-xs font-semibold text-slate-800">{course.title}</p><p className="mt-1 text-[9px] text-slate-400">{course.satisfies[0]}</p></div><button onClick={() => removeCourse(term.id, courseId)} aria-label={`Remove ${course.course} from ${term.label}`} className="text-slate-300 hover:text-rose-500"><X className="size-3.5" /></button></div></div>;
                })}{courseIds.length === 0 && <div className="grid min-h-40 place-items-center rounded-xl text-center"><div><Plus className="mx-auto size-5 text-slate-300" /><p className="mt-2 text-[11px] text-slate-400">Drop a course here</p></div></div>}</div>
              </div>;
            })}
          </div>
        </section>
    ...
    {customCourseOpen && <CustomCoursePanel terms={terms} onAdd={addCustomCourse} onClose={() => setCustomCourseOpen(false)} />}
  </>;
}
