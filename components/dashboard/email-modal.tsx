...
import type { DraftEmail, VerificationItem } from "@/lib/types";

export function EmailModal({ item, onClose }: { item: VerificationItem; onClose: () => void }) {
  ...
  useEffect(() => {
    void academicPlanningServices.uncertaintyHandler
      .draftEmail(item, { profile, transcript, targets, scenario })
      .then(setDraft);
  }, [item, profile, transcript, targets, scenario]);
      ...
      <div className="w-full max-w-2xl overflow-hidden rounded-2xl bg-white shadow-2xl">
        ...
        {!draft ? <div className="flex min-h-72 items-center justify-center gap-2 text-sm text-slate-500"><LoaderCircle className="size-4 animate-spin text-teal-600" /> Preparing a precise policy question…</div> : (
          <div className="p-5 sm:p-6">
            ...
            <div className="mt-4 grid gap-2 rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs sm:grid-cols-3">
              <div><span className="block text-[10px] font-bold uppercase tracking-wide text-slate-400">Course</span><strong className="mt-1 block text-slate-800">{draft.context.course}</strong></div>
              <div><span className="block text-[10px] font-bold uppercase tracking-wide text-slate-400">Institution</span><strong className="mt-1 block text-slate-800">{draft.context.institution}</strong></div>
              <div><span className="block text-[10px] font-bold uppercase tracking-wide text-slate-400">Term</span><strong className="mt-1 block text-slate-800">{draft.context.term}</strong></div>
              <div className="sm:col-span-3"><span className="block text-[10px] font-bold uppercase tracking-wide text-slate-400">Exact question</span><p className="mt-1 leading-5 text-slate-700">{draft.context.question}</p></div>
            </div>
