...
async def test_openai_client_rejects_quote_not_present_in_supplied_content() -> None:
    ...
    assert result.validation_issues == ["evidence_not_found: maximum of 105 lower-division credits"]
...
async def test_openai_client_bounds_source_text_before_request() -> None:
    ...
    context = _context().model_copy(update={"cleaned_source_text": "policy " + ("x" * 500)})
